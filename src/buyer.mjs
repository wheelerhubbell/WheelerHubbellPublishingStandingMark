...
export class StandingBuyer {
  ...
  async quoteCheck(terms,s,discovery){
    demand(terms.x402Version===2&&terms.extensions?.['whp-standing']?.info?.required===true,'BUYER_X402_BINDING_EXTENSION_REQUIRED');
    const {d,trust}=discovery??await this.discover();
    const quote=terms.extensions['whp-standing'].info.quote,cert=issuerAuthority(quote.protected.key_id,s.bounds.scope,s.bounds.jurisdiction,trust),q=openSeal(quote,'WHP-STANDING-QUOTE-v1',cert.public_key);
    ...
    demand(q.purchase_id===purchaseId(s,this.policy.root_pin)&&q.request_hash===hash(s)&&q.buyer_key===s.buyer_key,'BUYER_QUOTE_SUBMISSION_MISMATCH');
  ...
  async purchase(submission){
    ...
    if(row.payment){
      ...
      const authorization={from:this.policy.payer,to:t.payTo,value:t.amount,validAfter:String(at-1),validBefore:String(Math.min(at+t.maxTimeoutSeconds,q.payload.expires_at)),nonce:authorizationNonce(q)};
