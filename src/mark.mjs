...
export function assembleResult(row,privateKey,rootPin) {
  ...
  demand(hash(evaluate(row.submission,row.trust_bundle,rootPin,row.decision.evaluated_at,row.authority_evidence))===hash(row.decision),'RESULT_DECISION_REPLAY',503);
  const isMark=row.decision.outcome==='ESTABLISHED';
  const p={
    version:'WHP-STANDING-RESULT-v1.1',environment:trust.profile.environment,
    ...
    authority:row.trust_bundle,authority_evidence:row.authority_evidence,
    ...
    commerce:{quote:row.quote,payment_identity:row.payment_key,payment_payload:row.payment_payload,
      ...
      assessor:'WHP Standing deterministic Structured Passage evaluator 1.1.0'},
    retrieval:{purchase_path:'/v1/purchases/'+row.id,result_path:'/v1/purchases/'+row.id+'/result',
      registry_path:'/v1/registry/'+row.id,authentication:'Opaque 256-bit purchase capability; no buyer authentication signature',additional_charge:false},
  ...
  const type=isMark?'WHP-STANDING-MARK-v1.1':'WHP-STANDING-ASSESSMENT-v1.1';
